...
def projection_angle(x, d):
    ...
    Args:
        x: the symbol xp or yp .
        d: the de-center from the center of a sphere .
    Return:
        project_angle: theta or phi .
    """
    x_max = (1 + d) / d
    # This is the inverse stereographic projection from plane coordinate x to angle.
...
def pano2stereo(pic, distance=1.):
    ...
    # Pixel centers in the square stereographic face, expressed on the projection plane.
    ...
    for face in range(4):
        ...
        # Rotate the panorama by 90 degrees so the same projection math yields the next face.
...
def realign_bbox(center_x, center_y, width, height, face):
    if face == 3:
        face = -1
    def safe_atan(x):
        # Avoid division by zero at the stereographic plane boundary.
        if x == 2: return pi/2
        elif x == -2: return -pi/2
        else: return atan(4*x/(4-x**2))

    # Convert normalized face center to longitude, then undo the face's 90-degree offset.
    xp = 4*(center_x-0.5)
    phi = safe_atan(xp)
    phi = phi + face*pi/2
    if phi > 2*pi:
    ...
    def realign_border(center, line):
        # Project both box edges so the panorama box size follows angular distortion.
